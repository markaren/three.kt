...
class Sky : Mesh(BoxBufferGeometry(1000), ShaderMaterial()) {
    ...
    init {
        ...
        material.fragmentShader = SkyShader.fragmentShader
        material.vertexShader = SkyShader.vertexShader
        material.uniforms.putAll(cloneUniforms(SkyShader.uniforms))
...
private val SkyShader = Shader(

    uniforms = mutableMapOf(
        "luminance" to Uniform(1f),
        "turbidity" to Uniform(2f),
        "rayleigh" to Uniform(1f),
        "mieCoefficient" to Uniform(0.005f),
        "mieDirectionalG" to Uniform(0.8f),
        "sunPosition" to Uniform(Vector3(1f, 1f, -1f))
    ),
